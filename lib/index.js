...
/** Backup path for the original ui-conversation bundle (created before first patch). */
function backupPath(bundle) {
  return `${bundle}.whale-whisper.bak`;
}
...
/** Ensure the original bundle is backed up before we ever modify it. */
function ensureBackup(bundle) {
  const bak = backupPath(bundle);
  if (!fs.existsSync(bak)) {
    fs.copyFileSync(bundle, bak);
  }
  return bak;
}
...
function apply() {
  try {
    ...
    console.warn(`[whale-whisper] patch not applied (${error.message}) — restore with: cp "${backupPath(bundlePath())}" "${bundlePath()}"`);
  }
}
